...
rollback_log=/tmp/funding-release-042-rollback.log
...
rollback() {
    status=$?
    if [[ $status -ne 0 ]]; then
        echo "ACTIVATION_FAILED status=$status; restoring release-041"
...
curl -fsS --max-time 45 http://127.0.0.1:8000/health/ready \
    >/tmp/funding-release-042-ready.json

printf '%s\n' 'funding-pnl-v2-20260811-042' >.release
tar -czf /home/tolik1992s/funding-pnl-v2-20260811-042.tar.gz \
    --exclude='./.env' \
    --exclude='./.git' \
    --exclude='./.runtime' \
    --exclude='./.venv' \
...
trap - EXIT
echo "ACTIVATION_OK"
cat /tmp/funding-release-042-ready.json
