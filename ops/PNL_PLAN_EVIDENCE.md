...
| Enforce `PAPER_AUTOTRADE` and a UTC start boundary | Runtime health fields and runner tests. Settings reject a naive boundary, normalize any aware offset to UTC, reject blank simulator namespaces, and reject equal candidate/baseline namespaces whenever shared comparison is enabled | Proven |
...
| Venue-specific future funding events | Adapter schedule tests cover 1/4/8-hour intervals and settlement accounting. A release-073 fail-closed live-public probe at `2026-08-14T09:07:51Z` selected exact BTC/USDT-family perpetuals on Binance, Bybit, Gate, OKX, and Hyperliquid; every current next timestamp was future/present, history was strictly ordered and non-future, and recent event deltas matched venue metadata within `5s`. All symbols reported a next timestamp: Binance `671/671`, Bybit `775/775`, Gate `919/919`, OKX `30/30`, Hyperliquid `232/232`. Observed universes were Binance `1/4/8h`, Bybit `1/4/8h`, Gate `1/4/8h`, OKX `4/8h`, and Hyperliquid `1h` | Proven locally and release-073 live-public |
...
| Daily Telegram report shows day and current-simulator totals | Telegram tests plus DB-backed, non-sending previews prove day/total accounting, runner evidence, separate candidate/baseline sections, and that a real funding event changes the reported total. A repeated scheduler cycle for the same local report date sees the committed unique `sent` ledger row and does not send or insert again. UTC query bounds are derived from consecutive `Europe/Kyiv` local midnights; regressions prove the 2026 spring DST day is 23 hours and the autumn day is 25 hours, so neither duplicate nor missing UTC hours distort day PnL. A release-073 fail-closed probe at `2026-08-14T09:08:23Z` called only Telegram `getMe/getChat`, redacts the token on every path, and proved the configured bot, exact private chat, and `00:00 Europe/Kyiv` schedule reachable from the VM without sending a message. Telegram Bot API has no client idempotency key, so this evidence deliberately claims post-commit deduplication rather than impossible crash-atomic exactly-once delivery | Proven locally and release-073 live-public |
...
| Historical candidate passes the economic comparison checks | Candidate `+$20.7234382435`; strict baseline `-$83.1268879021`; snapshot max drawdown `0.102224%` versus `2.000771%`; higher median monthly PnL; 2/3 profitable snapshot windows; 721 exact shared timestamps; event/snapshot PnL error below `$0.01`. The 10% gate is sign-safe: a positive baseline requires candidate PnL `>= baseline × 1.10`, while a non-positive baseline requires strictly positive candidate PnL, so a merely smaller loss cannot pass. Validation uses three chronological equal-duration OOS thirds: the first two are half-open and the last is end-inclusive, so boundary snapshots are counted exactly once and window PnL conserves total PnL. Acceptance separately requires snapshot-backed window evidence, preventing event fallback from passing the gate | Proven historical economics only |
...
| 50-minute post-boundary checkpoint | The explicit-version canary audit observed 92 exact shared v31 snapshot pairs from `08:40:03Z` through `09:30:31Z`; maximum gap was `76.400739s`, pending snapshots/incidents/carry-in/restarts were zero, and both latest accounting invariant errors were exactly `0E-18`. All five venues remained healthy with complete funding/order-book coverage, zero stale books, zero runner errors, and fresh ticker/order-book WebSockets (worst observed stream age `2.071s`). Candidate remained flat. Baseline held two distinct canonical exposures: the original COTI position and a separate HOME Gate-short/Bybit-long position opened at `09:13:51Z`, with no duplicate or overlapping exposure intervals. The canary gate remained false only because its 72-hour duration had not elapsed | Proven timed checkpoint |
...
| Final acceptance cannot pass without continuous shared telemetry | Core comparison and tracked operator audits fail closed when either comparable snapshot series is empty, max gap exceeds `300s`, snapshot-derived risk/window sources are absent, or an operator omits the exact clean boundary/candidate/baseline namespaces. This prevents a future gate from silently auditing invalidated v30 defaults. The runtime pins every open-position market, rejects an unmarkable shared snapshot before either ledger mutates, and reports readiness from fully completed shared snapshots. Local verification is 169 tests plus clean Ruff and mypy over 88 source files | Proven implementation; timed window pending |
