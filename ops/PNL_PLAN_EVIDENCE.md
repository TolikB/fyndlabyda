...
| Daily Telegram report shows day and current-simulator totals | Telegram tests plus DB-backed, non-sending previews prove day/total accounting, runner evidence, separate candidate/baseline sections, and that a real funding event changes the reported total. A release-073 fail-closed probe at `2026-08-14T09:08:23Z` called only Telegram `getMe/getChat`, redacts the token on every path, and proved the configured bot, exact private chat, and `00:00 Europe/Kyiv` schedule reachable from the VM without sending a message | Proven locally and release-073 live-public |
