...
PAPER_AUTOTRADE_START_UTC=2026-08-14T01:15:00Z
PAPER_SIMULATION_VERSION=v27-oos-candidate
PAPER_BASELINE_SIMULATION_VERSION=v27-oos-baseline
```
...
curl 'http://127.0.0.1:8000/analytics/attribution?simulation_version=v27-oos-candidate'
...
The mark-to-market, restart-safe, reverse-route-deduplicated v27 canary starts
with release `funding-pnl-v2-20260814-068`. Its clean evidence boundary and
enforced autotrade boundary are both `2026-08-14T01:15:00Z`; use that exact
timestamp as the `--start` value below. Run the read-only audit inside the
deployed application container after the relevant deadline:

```bash
# Earliest useful run: 2026-08-17T01:16:00Z.
...
docker compose -p funding_arbitrage_paper exec -T app \
  ...
  --start 2026-08-14T01:15:00Z \
  --gate canary \
  --timeout 45

# Earliest useful run: 2026-09-13T01:16:00Z.
cd /opt/funding_arbitrage_paper
docker compose -p funding_arbitrage_paper exec -T app \
  python scripts/paper_acceptance_audit.py \
  --start 2026-08-14T01:15:00Z \
