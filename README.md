...
Market-neutral funding/basis research and execution system. It normalizes public
market data from Bybit, Gate.io, OKX, Binance, and Hyperliquid and ranks funding,
spot/perpetual, perp/perp, and dated-futures opportunities. Paper mode remains
the default; the separately armed live mode uses authenticated trade-only API
credentials and hard reconciliation/risk interlocks.
...
The clean shared-feed OOS candidate is versioned as `v16-oos-candidate`. It requires typed,
...
GET  /analytics/attribution?simulation_version=v16-oos-candidate
...
## Real-money execution branch

The optional live mode is disabled by default and is separate from paper mode.
It uses authenticated CCXT adapters for Bybit, Gate, OKX, Binance, and
Hyperliquid, records intent and a pending client order ID before every API call,
uses bounded IOC limits, compensates incomplete two-leg entries, treats network
timeouts as unknown rather than failed, and stops new entries on any private
state mismatch. Restart readiness requires reconciliation of durable positions,
real derivative positions, spot inventory, and open orders.

Live mode also enforces venue/asset/strategy allowlists, position and aggregate
notional limits, free-collateral reserve, one-way isolated 1x defaults, daily
loss and drawdown limits, typed fresh order books, exact settlement-aware entry
and exit decisions, a persistent operator kill switch, and actual-equity daily
Telegram reports. It deliberately does not implement spot borrowing.

Prepare credentials with `.env.live.example` and follow
`ops/LIVE_TRADING_RUNBOOK.md`. Do not enable live mode until paper validation and
the documented small-capital canary are complete.
